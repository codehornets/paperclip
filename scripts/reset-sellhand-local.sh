...
node "$repo_root/company-packages/sellhand/scripts/clear-local-state.mjs" reset "$runtime_root" "$repo_root" "$instance_id"
SELLHAND_PAPERCLIP_HOME="$runtime_root" SELLHAND_PAPERCLIP_INSTANCE="$instance_id" "$repo_root/scripts/bootstrap-sellhand-local.sh"
